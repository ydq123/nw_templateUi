                    ...
                    <div class="flex-1">
                      <div class="mb20">
                        ...
                        <div class="flex al-c f13 txt-666">
                          <div class="mr5  pr5 pt5 clamp1" style="width: 65%;">
                            意见：{{colItme.msg||'同意'}}
                          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
              ...
              if (retBoolean) {
                if (ret.length > 0) {
                  ...
                  ret.forEach((item, index) => {
                    ...
                    /* 获取用户id*/
                    item.colTransTrackInfo.forEach((item2, index) => {
                      userIdArr.push(item2.actorId);
                    });
                  });
                  console.log(sumMin)
                  ...
                  this.nodeList = ret;
                  /* 获取用户手机号码*/
                  this.queryUserListByUserIdsFun(userIdArr);
                }
              }
            }
          })
      ...
      /* 获取人员基本信息*/
      queryUserListByUserIdsFun: function(arr) {
        // this.$textLoading("加载中"); mobilePhone
        // console.log(arr)
        let params = [];
        queryUserListByUserIds(arr)
          .then(ret => {
            // console.log(ret);
            if (ret) {
              // retBoolean=true(数组)，false(不是数组)
      ...
      callPhone: function(data) {
        console.log(data);
        var arr = this.userInfoArr.filter(itme => itme.userId == data.actorId) || [];
        console.log(arr[0].mobilePhone)
        if (arr.length > 0) {
          this.myJssdk.phoneCall({
            phoneNumber: arr[0].mobilePhone
          });
        } else {
