...
<template>
  <div>
    <div class="yw-topinput bg-f5">
      <slot name="topInput">
        ...
        <div class="top-input input-row al-c" v-show="topInput">
          <div class="input-box flex-1 input-row">
            <i
              class="iconfont icon-sousuo f14 text-99 ml10 mr5"
              @click="inputTopBtn"
            ></i>
            <input
              id="tapInput"
              type="text"
              class="input flex-1 f14"
              v-model="searchValue"
            ...
            <i
              ...
              class="iconfont icon-shanchu3 f15 pl5 mr5 gray9"
              @click="cancelSearch"
            ></i>
          </div>
        </div>
      ...
      <slot name="fousInput">
        ...
        <div class="top-input input-row al-c">
          <div class="input-box flex-1 input-row">
            ...
            <i
              v-if="isShowScanIcon"
              class="iconfont icon-sao f15 pl5 mr10 gray287"
              @click="scanHandle"
            ></i>
          </div>
          <div
            class="txt-1e8 f14 pl10"
            @click="cancellBtn"
      ...
      <slot name="taskSreenSort">
        <div class="input-row input-srot borderTopE8">
          <div class="flex-1 text-left gray6 pl15 f16 verticle-center">
            共{{ listNub }}条记录
          </div>
          <div
            class="task-top-sreen-btn pl15 pr15 verticle-center"
            m="click"
            @click="changeSort"
            v-if="isSort"
          >
            ...
            <i
              class="iconfont icon-qianjin-copy f10 gray9"
              :class="{ showRotate: sortData.showPop }"
            ></i>
          </div>
          <div
            class="task-top-sreen-btn pl15 pr15 verticle-center"
      ...
      <div v-if="sortData.showPop" class="bg-white">
        <div v-for="(sortItem, sortIndex) in sortData.list" :key="sortIndex">
          <div
            class="f16 p15 gray3 borderTopE8 input-row"
            @click="changeSortItem(sortIndex)"
          >
            <div class="flex-1 text-left">{{ sortItem.text }}</div>
            <i
              class="iconfont pos-a gray287 icon-gou"
              v-if="sortIndex == sortData.dtIndex"
            ></i>
          </div>
        </div>
      </div>
    ...
    <div
      class="zdc"
      :style="{ 'z-index': mTopZindex }"
      v-show="showPop"
      @click.stop="zdcBtnShow"
    ></div>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    isTopInput: {
      // 控制头部搜索点击事件是否显示搜索框，默认为false不显示，true显示
      type: Boolean,
      default: false
    },
    mTop: {
