					...
					<div v-for="(sItem,sIndex) in sreenData.sreenTodoItem" :key="sIndex">
						...
						<div class="clearfix pl15">
							<div v-for="(value,index) in sItem.sreenList" :key="index" class="text-center col-xs-4" @click="changSreenItem(sIndex,index)">
								<div @click="" class="sreen-item border_1_dc f14 pt10 pb10 pl15 pr15 mr15 mb15"
								 :class="[value.isSelect ? 'sreen-item-cur' : '']">
									{{value.text}}
								</div>
							</div>
						</div>
			...
			changSreenItem: function(sIndex,item){
				if(this.curTabIndex == 0){
					if(this.sreenData.qfDdx){
						this.sreenData.sreenTodoItem[sIndex].sreenList[item].isSelect = !this.sreenData.sreenTodoItem[sIndex].sreenList[item].isSelect;
					}else{
				...
				if(this.curTabIndex == 1){
					if(this.sreenData.qfDdx){
						this.sreenData.sreenDoneItem[sIndex].sreenList[item].isSelect = !this.sreenData.sreenDoneItem[sIndex].sreenList[item].isSelect;
					}else{
