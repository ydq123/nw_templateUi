...
export const NWbaseMinxin = {
  ...
  methods: {
    ...
    $baseTimeGetDateDiff: function(publishTime, timeStatus) {
      ...
      } else if (this.$baseTimeDate('d', val) === this.$baseTimeDate('d', now) && this.$baseTimeDate('Y', val) ===
        this.$baseTimeDate(
      ...
      } else if (parseInt(this.$baseTimeDate('d', val)) + 1 === parseInt(this.$baseTimeDate('d', now)) && this
        .$baseTimeDate(
    ...
    //判断类型number、string、boolean、undefined、null、arr，obj、function jsonStr
